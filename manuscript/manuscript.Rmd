...
Code availability: All data used in the meta-analyses is available [here](https://github.com/awconway/zhf-review).
...
Zero-heat-flux thermometry is an alternative non-invasive method that allows for continuous monitoring of core temperature. Zero-heat-flux sensors are placed on the lateral surface of the forehead and is initially warmed to equilibrate the temperature of the skin surface to the underlying core tissues. Equipped with a thermal insulator, the zero-heat-flux sensor eliminates heat loss to the environment to allow for changes in core temperature to be directly reflected by a change in skin surface temperature.[@eshraghi2014] Originally developed in the 1970s, zero-heat-flux technology was recently implemented in the the 3M^TM^ Bair Hugger^TM^ Temperature Monitoring System (3M^TM^, St Paul, MN), as a single-use, disposable sensor. This device was previously known as the SpotOn Temperature Monitoring System. Another device on the market is the T-core sensor from Dräger. 

The agreement between temperature measurements from zero-heat-flux thermometers and core as well as other peripheral thermometers has been investigated in multiple studies over the last five years. The 3M^TM^ Bair Hugger^TM^ Temperature Monitoring System, has been the zero-heat-flux device used in most of these recent studies. Appraisal and synthesis of the results would aid clinicians in deciding the appropriate circumstances in which this particular zero-heat-flux thermometer may be used. We aimed to determine if 3M^TM^ Bair Hugger^TM^ Temperature Monitoring System has clinically acceptable accuracy and precision relative to established core and peripheral temperature measurement devices. Accuracy is defined as the average difference between temperature measurements from the zero-heat-flux and comparator device and precision as the variance (standard deviation) in the differences.
...
It should be noted that we did not include studies that used the Temple Touch Pro because it is not strictly a zero-heat-flux device. This is a new thermometer that is similar to the zero-heat-flux device in that it is placed cutaneously, but the underlying technology is different.[@evron2017evaluation] Likewise, studies that evaluated the Drager Tcore device were not included because this review focused specifically on the 3M^TM^ Bair Hugger^TM^ Temperature Monitoring System.
...
ft <- data_core %>% 
  ...
  dplyr::mutate(N= tidyr::replace_na(N, 0)) %>% 
    dplyr::mutate(lower_temp= tidyr::replace_na(lower_temp, 0)) %>% 
  dplyr::mutate(upper_temp= tidyr::replace_na(upper_temp, 0)) %>% 
    ...
    dplyr::mutate(temp_range = glue::glue("{lower_temp}-{upper_temp}")) %>%
     ...
     dplyr::select(Year, Study, code, patients, perc_male, age, comparison, n, N, 
           temp_range) %>%
      flextable()
...
ft <- compose(ft, i=~code=="US", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("US", filetype = "png"),
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="FI", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("FI", filetype = "png"),
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="FR", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("FR", filetype = "png"),
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="BE", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("BE", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="ES", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("ES", filetype = "png"), 
    width = .20, height = .15))
)
...
ft <- compose(ft, i=~code=="GB", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("GB", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="JP", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("JP", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="IT", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("IT", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="DE", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("DE", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose(ft, i=~code=="CA", j = "code",
  value = as_paragraph(as_image( 
    src = flagon::flags("CA", filetype = "png"), 
    width = .20, height = .15))
)

ft <- compose( ft, j = "n",   value = as_paragraph(as_chunk(format(n)), "    ",
...
ft %>% 
  fontsize(size = 10) %>% 
  fix_border_issues() %>% 
  add_header_row(values = c("Study","Participants", "Temperature measurements"),  colwidths = c(4, 2 , 4)) %>% 
  ...
  bg(i=1, j=7:10, part="header", bg="#930093") %>% 
  color(i=1, j=7:10, part="header", color="white")  %>% 
  empty_blanks() %>% 
  set_header_labels(Year ="", Study = "", code = "",
                    patients = "", age= "Age (years)",
                    comparison = "", n = "Participants",
                    N = "Measurements", temp_range = "Range (°C)") %>% 
  border_remove() %>% 
...
hline( i =~Year=="2014", part = "body", 
            border = officer::fp_border(color="black", width = 1))  %>% 
  bg(j=~comparison + temp_range + n+N, bg="lightgray") %>% 
  ...
  width(j="n",width = 1.75) %>%
  width(j="N",width = 1.81) %>%
...
```{r fig.cap="Fig. 1", include=params$include_figs, eval=params$include_figs}
...
```{r fig.cap="Fig. 2", include=params$include_figs, eval=params$include_figs}
RoB <- data_core %>%
  dplyr::bind_rows(data_SL) %>%
  dplyr::bind_rows(data_NPA) %>%
...
robvis::rob_traffic_light(data = RoB, tool = "QUADAS-2")
```

\pagebreak

```{r fig.cap="Fig. 3", include=params$include_figs, eval=params$include_figs}
bias=data_core$bias
s2_unb = data_core$s2
