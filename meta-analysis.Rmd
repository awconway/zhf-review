...
data_core <- data %>% #need to adjust for Eshragi, want overall only
  filter(comparison=="PA"|comparison=="Bladder"|comparison=="Eso" | comparison =="Rectal"|comparison =="Ax"|comparison =="Iliac")

data_core_lowrisk <- data_core %>% # need to adjust for eshragi, want overall only
  filter(RoB_selection == "low" & RoB_spoton == "low" & RoB_comparator == "low" & RoB_flow =="low" & (comments != "Intraoperative, off cardiopulmonary bypass" | comments != "Postoperative"))

data_core_op <- data_core %>% 
  filter(clinical_setting=="Intraoperative" & (clinical_setting != "Postoperative" | clinical_setting != "Intra- and Postoperative"))
