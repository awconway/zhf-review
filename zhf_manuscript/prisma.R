...
annotation_custom(grob = 
 ...
 geom_fit_text(xmin = second_x_min, xmax=second_x_max,
                ...
                label= glue::glue(full_text_excluded, ' studies excluded'),
                 reflow = TRUE, colour = text_colour) +
  geom_fit_text(xmin = second_x_min, xmax=second_x_max,
                ymin=full_excluded_y_max-10, ymax=full_excluded_y_max-7,
                label= glue::glue(wrong_intervention, ' wrong intervention'),
                reflow = TRUE, colour = text_colour) +
    geom_fit_text(xmin = second_x_min, xmax=second_x_max,
                  ymin=full_excluded_y_max-13, ymax=full_excluded_y_max-10,
                  label= glue::glue(wrong_intervention, ' wrong comparator'),
                  reflow = TRUE, colour = text_colour) +
    geom_fit_text(xmin = second_x_min, xmax=second_x_max,
                  ymin=full_excluded_y_max-16, ymax=full_excluded_y_max-13,
                  label= glue::glue(wrong_intervention, ' wrong design'),
                  reflow = TRUE, colour = text_colour) +
...
ggsave(here::here("zhf_manuscript","figure_one.tiff"), device = "tiff", 
       width=7.5, height=7, units='in', dpi=300, compression = 'lzw')
