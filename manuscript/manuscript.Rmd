...
- name: Kristina Chang
  address: Toronto General Hospital, UHN
- name: Peter Collins
  address: Toronto General Hospital, UHN
- name: Matteo Parotto
  address: Toronto General Hospital, UHN
...
library(tidyverse)
library(knitr)
library(bama) # https://github.com/awconway/bama
...
df %>% 
  rename("Mean bias" = bias_mean, "Variance" = sd2_est, "Tau-squared" = tau_est, "Lower bound for 95% limit of agreement" = LOA_L, "Upper bound for 95% limits of agreement" = LOA_U, "Outer confidence interval for lower 95% limits of agreement" = CI_L_rve, "Outer confidence interval for upper 95% limits of agreement" = CI_U_rve) %>% 
    gt() %>% 
  tab_header(
...
Results from this systematic review have important implications for practice. Clinicians should consider the potential that a temperature measurement from a zero-heat-flux thermometer could be as much as one degree Celcius higher or lower than core temperature. It was reassuting that results of our sensitivity analysis restricted to studies judged to be at low risk of bias using the QADAS-2 tool were similar. As such, it is vital for clinicians considering using this device to first determine if differences in temperature smaller than this magnitude would be important for the given clinical situation. If so, then it may not be appropriate to substitute the zero-heat-flux device in place of a core thermometer. 

Our estimates of the accuracy of zero-heat-flux thermometers are similar to results from a previous meta-analysis that compared other peripheral thermometers, such as sublingual and temporal artery devices, with core temperature measurements [@niven2015accuracy]. However, this previous meta-analysis used a statistical approach that did not incorporate the magnitude of heteroogeneity in results between studies or sampling error. As such, it is possible that the zero-heat-flux thermometer may still more precise than other peripheral thermometers.
...
Although comparisons with core temperture are the most important to evaluate, we included studies that compared zero heat flux and peripheral temperature monitoring devices in our review because clinicians may be interested to determine if this new technology is sufficiently accurate to replace other ways non-invasive monitor temperature. Also, there may be situations where it is not practical to expose research participants to the potential additional risk of invasive core temperature monitoring for the purposes of evaluating the new temperature monitoring device when it is standard practice for only peripheral temperature monitoring to be used. Whether or not the zero-heat-flux thermometer is sufficiently accurate to be used in place of nasopharyngeal thermometers is unclear. Population limits of agreement calculated from four studies were broad, spanning from one degree less than zero heat flux temperature to one degree higher. Further studies are therefore required.
...
Many studies in this review analysed a large number of measurements of temperature with relatively small sample sizes. Importantly, the approach we used for our meta-analysis takes this into account. By using robust variance estimation, weights for pooling estimates in the meta-analyis become proportional to the number of patients not the total number of measurements.[@tipton2017framework]

The accuracy of zero heat flux temperature monitoring has been evaluated in various intra-operative contexts as well as in the intensive care unit setting. Our subgroup analyses did not indicate that the accuracy of this temperature monitoring device was any more effective in a particular setting. Only one study included in our systematic review included pediatric patients. Additional studies to evaluate the accuracy of this device in children may be warranted to increase confidence. 
